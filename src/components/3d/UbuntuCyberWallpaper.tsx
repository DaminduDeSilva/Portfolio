...
function DataNodes({ count = 3000 }) {
  ...
  const { positions, colors, randoms } = useMemo(() => {
    ...
    const baseColor1 = new THREE.Color("#ff8a4d"); // Ubuntu Orange Glow
    const baseColor2 = new THREE.Color("#8a3b88"); // Ubuntu Aubergine Glow
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Blend between Ubuntu colors randomly
...
export default function UbuntuCyberWallpaper() {
  return (
    <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
      <Canvas camera={{ position: [0, 0, 10], fov: 60 }} dpr={[1, 2]}>
        {/* Deep tech background */}
        <color attach="background" args={["#220018"]} />
